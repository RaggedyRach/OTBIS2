...
namespace OTBIS.Web.Services
{
    public class StoreReportService
    {
        ...
        public async void ExportByDate(string name, string type, Radzen.Query query)
        {
            navigationManager.NavigateTo(query != null ? query.ToUrl
                ($"/export/name/{type}") : $"/export/name/{type}", true);
        }
